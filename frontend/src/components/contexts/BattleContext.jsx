...
function reducer(state, action) {
	switch (action.type) {
		case "SET_ROOM":           return { ...state, ...action.payload, error: null };
		case "OPPONENT_JOINED":    return { ...state, opponentName: action.payload.guestName, opponentConnected: true };
		case "SET_SETTINGS":       return { ...state, settings: { ...state.settings, ...action.payload } };
		case "SET_SELF_READY":     return { ...state, selfReady: true };
		case "SET_OPPONENT_READY": return { ...state, opponentReady: true };
		case "SET_COUNTDOWN":      return { ...state, phase: "countdown", countdown: action.payload };
		case "BATTLE_START":       return { ...state, phase: "battle", question: action.payload.question, startTime: action.payload.startTime, duration: action.payload.duration, countdown: null };
		case "TICK":               return { ...state, elapsed: action.payload.elapsed, remaining: action.payload.remaining };
		case "SET_SUBMISSION":     return { ...state, submissionResult: action.payload };
		case "SET_OPP_SUBMISSION": return { ...state, opponentSubmission: action.payload };
		case "SET_OPP_TYPING":     return { ...state, opponentTyping: true };
		case "BATTLE_RESULT":      return { ...state, phase: "result", battleResult: action.payload };
...
export function BattleProvider({ children }) {
	...
	return (
		<BattleContext.Provider value={{ state, actions, socket: socketRef }}>
			{children}
