...
function UsernameModal({ onSubmit }) {
	...
	const handleSubmit = () => {
		if (!gh.trim() && !lc.trim()) { setError("Enter at least one username."); return; }
		setError("");
	...
	return (
		<div className="modal-overlay">
			<div className="modal-card">
				...
				<p className="modal-sub">Enter your GitHub and LeetCode usernames to load your activity and AI mentor.</p>
				...
				<div className="modal-fields">
					<div className="modal-field">
						...
						<input className="modal-input" type="text" placeholder="e.g. Aditya-Pandey-GH"
							value={gh} onChange={e => setGh(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} />
					</div>
					...
					<div className="modal-field">
						...
						<input className="modal-input" type="text" placeholder="e.g. aditya_pandey"
							value={lc} onChange={e => setLc(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} />
					</div>
				</div>

				{error && <p className="modal-error">{error}</p>}
				<button className="modal-btn" onClick={handleSubmit}>Load Dashboard →</button>
				<p className="modal-skip" onClick={() => onSubmit("", "")}>Skip for now</p>
			</div>
		</div>
	);
}
	...
	useEffect(() => {
		...
		if (ghUser) {
			fetch(`https://github-contributions-api.jogruber.de/v4/${ghUser}?format=nested&y=${year}`)
				.then(r => r.json()).then(setGhData).catch(() => {});
		}
		if (lcUser) {
			fetch(`https://alfa-leetcode-api-adi.vercel.app/${lcUser}/calendar?year=${year}`)
				.then(r => r.json())
				.then(d => { try { setLcData(JSON.parse(d?.submissionCalendar)); } catch {} })
				.catch(() => {});
		}
	}, [ghUser, lcUser]);
					...
					<div className="welcome-block">
						<div className="welcome-block-left">
							...
							<p className="welcome-desc">
								Learning progress and today's personalized study plan
							</p>
						</div>
