	...
	return (
		<div className="heatmap-block">
			<div className="heatmap-header">
				<span className="heatmap-title" style={{ color: accent }}>
					{title}
				</span>
						...
						return (
							...
							>
								...
								<div className="bars-pair">
									<div className="bar gh-bar" style={{ height: `${ghH}%` }} title={`GitHub: ${m.ghTotal}`} />
									<div className="bar lc-bar" style={{ height: `${lcH}%` }} title={`LeetCode: ${m.lcTotal}`} />
								</div>
...
export default function ActivityGraph({ ghUserName, lcUserName }) {
		...
		const promises = [
			...
			lcUserName &&
				fetch(`https://alfa-leetcode-api-adi.vercel.app/${lcUserName}/calendar?year=${year}`)
					...
					.then((d) => {
						try {
							return JSON.parse(d?.submissionCalendar);
						} catch {
							return null;
						}
					})
			...
			<div className="ag-topbar">
				<div className="ag-title-row">
					...
					<div className="ag-year-nav">
						<button className="ag-yr-btn" onClick={() => setYear((y) => y - 1)} disabled={year <= 2020}>
							‹
						</button>
						<span className="ag-year">{year}</span>
						<button className="ag-yr-btn" onClick={() => setYear((y) => y + 1)} disabled={year >= curYear}>
							›
						</button>
					</div>
				</div>
