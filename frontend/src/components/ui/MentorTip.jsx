...
Rules: No bullet points. No emojis. No headers. Speak directly using "you". Reference the actual numbers.`;
	...
	return data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() ?? "Keep pushing. Consistency compounds.";
}
...
export default function MentorTip({ ghData, lcData, ghUserName, lcUserName }) {
	...
	return (
		<div className="mentor-block">
			...
			<div className="mentor-header">
				...
				<div className="mentor-header-right">
					...
					<div className="voice-picker-wrap">
						<button className="voice-picker-btn" onClick={() => setShowVoicePicker((v) => !v)} title="Change voice">
					...
					{revealed && (
						<>
							...
							{!audioLoading && audioObj && (
								<button className={`mentor-audio-btn ${playing ? "active" : ""}`} onClick={toggleAudio}>
									{playing ? (
										<>
											<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor">
			...
			{/* States */}
			{!revealed && !loading && <p className="mentor-prompt-text">Want a tip based on your code activity?</p>}
			...
			{revealed && tip && <blockquote className="mentor-tip-text">{tip}</blockquote>}

			{tipError && <p className="mentor-error">{tipError}</p>}
			{audioError && <p className="mentor-error audio">{audioError}</p>}

			{/* CTA */}
			<button className={`mentor-cta ${loading ? "disabled" : ""}`} onClick={handleGetTip} disabled={loading}>
