...
import { EXTENSION_TO_LANGUAGE } from "../helpers/CodeEditorConstants";

export default function TabBar({ files, activeFileId, setFiles, setActiveFileId }) {
	...
	function saveRename(fileId) {
		...
		setFiles(files.map((f) => (f.id === fileId ? { ...f, name: editingName.trim(), language: detectedLanguage } : f)));
			...
			{files.map((file) => (
				...
				>
					{editingId === file.id ? (
						...
						<Text flex={1} noOfLines={1} mr={2} onDoubleClick={(e) => startRename(file, e)} title="Double-click to rename">
							{file.name}
						</Text>
					)}
